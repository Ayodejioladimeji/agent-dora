...
export async function GET(request: NextRequest, { params }: { params: { platform: string } }) {
  ...
  try {
    ...
    const response = NextResponse.redirect(`${process.env.NEXTAUTH_URL}/callback?platform=${platform}&success=true`)
    response.cookies.delete(`oauth_state_${platform}`)
    return response
  } catch (error) {
    console.error("OAuth callback error:", error)
    // return NextResponse.json({ error: "my auth error" }, { status: 500 })
    return NextResponse.redirect(`${process.env.NEXTAUTH_URL}/callback?platform=${platform}&success=false`)
  }
}
